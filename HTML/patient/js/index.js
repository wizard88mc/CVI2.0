identificationType = childIdentificationType;
		...
		if (data.TYPE == "OFFSET_CALCULATION") {
			...
			if (data.TODO == "true") {
				...
				if (data.MANDATORY == "true") {
					...
					var dialog = $('<div>').attr('id', 'dialogAskSynch')
						.attr('title', 'Sincronizzazione')
						.appendTo('#divMainContent');

					$('<p>').text('Computer non ancora sincronizzato con il ' + 
						'server. Eseguire sincronizzazione ora?')
						.appendTo(dialog);
					$('<p>').text('Attenzione: se non si effettua la ' + 
						'sincronizzazione non sarà possibile giocare..')
						.appendTo(dialog);
				...
				// can decide if sync another time or not
					...
					$('<p>').text('Il computer non è stato sincronizzato ' +
						'recentemente e sarebbe opportuno effettuarla per avere' + 
						' dati più precisi.').appendTo(dialog);
...
function putInWaitingToStart() {
	...
	websocket.onmessage = function(message) {
		...
		if (data.TYPE == "GAME") {
			...
			websocket = null;
			setTimeout(function() {
				if (data.DATA == "CATCH_ME") {
					location.replace('../catchMe/index.html');
				}
				else if (data.DATA == "HELP_ME") {
...
function initPage() {
	...
	if (getFromSessionStorage("permission") == "DOCTOR") {
		...
		openWebSocket(childClientWebsocketPort);
	}
		...
		$('<h1>').text('Ciao ' + patientName).addClass("title")
			.appendTo('#divMainContent');
		$('<h1>').text('A cosa giochiamo oggi???')
			.appendTo('#divMainContent');
		
		var divContainer = $('<div>').attr('id', 'divButtonsGames')
			.appendTo('#divMainContent');

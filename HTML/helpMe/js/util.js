...
istantiateLevel: function(level) {
    ...
    if (familySound[targetFamily].audioBagCompleted) {
    	utilsNamespace.addSoundSource($('<audio>').attr('id', 'audioBagComplete')
    			.appendTo('#divSounds'),familySound[targetFamily].audioBagCompleted);
    }
    
    utilsNamespace.addSoundSource($('<audio>').attr('id', 'audioObjectNotInserted')
    		.appendTo('#divSounds'), familySound[targetFamily].audioObjectNotInserted);

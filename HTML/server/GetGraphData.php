...
if ($fileDelta != 0) {
	...
	while (!feof($fileDelta)) {
		...
		if (count($components) > 1) {
			...
			$valueForDeltaTouch = str_replace($replace, "", $components[1]);
			if ($valueForDeltaTouch != null) {
				$valueForDeltaTouch = (int)$valueForDeltaTouch;
			}
			$valueForDeltaEyes = str_replace($replace, "", $components[2]);
			if ($valueForDeltaEyes != null) {
				$valueForDeltaEyes = (int)$valueForDeltaEyes;
			}
		
			$informations = array(
				"DELTA_TOUCH" => $valueForDeltaTouch,
				"DELTA_EYE" => $valueForDeltaEyes,

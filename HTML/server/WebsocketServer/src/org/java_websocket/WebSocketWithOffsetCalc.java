...
public abstract class WebSocketWithOffsetCalc extends BaseManager {
    ...
    WebSocketWithOffsetCalc(String clientType, int port) 
            ...
            if (result.isEmpty()) {
                ...
                try {
                    ...
                    if (difference > maxDifference) {
                        ...
                        packetMaybeCalculation.put(BaseManager.MESSAGE_TYPE, 
                                BaseManager.OFFSET_CALCULATION);
                        ...
                        packetAlreadySync.put(BaseManager.MESSAGE_TYPE, 
                                BaseManager.OFFSET_CALCULATION);
    ...
    private void performOffsetCalculation(JSONObject packet, 
            long millisecondsReception) {
        ...
        if (operationComplete) {
            ...
            try {
                valuea12 = Double.parseDouble(MathUtils.twoDigits
                        .format(offsetCalculator.getFinalA12()).replace(",", "."));
                valueb12 = Double.parseDouble(MathUtils.twoDigits
                        .format(offsetCalculator.getFinalB12()).replace(",", "."));
            }
            catch(NumberFormatException exc) {
                System.out.println("Errore format: " + exc.toString());
            }
            
            int newMachineID = dbManager.insertNewMachineOffset(valuea12 + "," + 
                    valueb12, machineID);

            JSONObject packetToSend = new JSONObject();
            packetToSend.put(BaseManager.MESSAGE_TYPE,
                    BaseManager.OFFSET_CALCULATION_COMPLETE);
